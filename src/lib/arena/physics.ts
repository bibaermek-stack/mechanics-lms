...
import { DRIVE_FORCE, KICK_COOLDOWN, KICK_IMPULSE, KICK_REACH, PITCH, resetPositions } from "./pitch";
 ...
 */
function collide(a: Disc, b: Disc, t: number): CollisionReport | null {
  const dx = b.x - a.x;
  const dy = b.y - a.y;
...
export function step(state: MatchState, h: number, inputs: Map<string, Input>, config: MatchConfig) {
  ...
  for (const d of state.discs) {
    ...
    if (d.kind === "player") {
      ...
      if (mag > 1e-6) {
        ...
        d.vx += ((DRIVE_FORCE * ux) / d.m) * h;
        d.vy += ((DRIVE_FORCE * uy) / d.m) * h;
      }
      if (input.kick && ball) {
  ...
  for (let i = 0; i < state.discs.length; i++) {
    for (let k = i + 1; k < state.discs.length; k++) {
      const report = collide(state.discs[i], state.discs[k], state.t);
      if (report) state.lastCollision = report;
    }
  }

  for (const d of state.discs) walls(d);
